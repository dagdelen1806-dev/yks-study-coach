...
export default function BookContentScanner({ book, onClose, onSaved, onAddAnother, intro, withCoverStep = false }: {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      const images = await Promise.all(pages.map((page) => compressImage(page.file, { rotate: page.rotate, crop: page.crop, maxBytes: perPageBytes, maxDimension: pages.length <= 2 ? 2400 : 2000, enhance: "document" })));
